...
import { createLiveQueryCollection, eq, gt, gte, lt, isNull } from '@tanstack/db'
import type { E2ETestConfig } from '../types'
import { createDeduplicationCounter } from '../utils/helpers'

export function createDeduplicationTestSuite(getConfig: () => Promise<E2ETestConfig>) {
    ...
    describe('Race Conditions', () => {
      ...
      it('should not corrupt data with concurrent queries', async () => {
        ...
        const queries = [
          ...
          createLiveQueryCollection((q) =>
            q.from({ user: usersCollection }).where(({ user }) => eq(user.isActive, true))
          ),
          createLiveQueryCollection((q) =>
